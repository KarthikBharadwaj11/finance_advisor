...
4. Recursive function:
   def _split_text_recursive(
    ...
    This is the base case. It checks whether the length of the text is smalelr than the chunk size.
    if len(text) <= chunk_size:
        return [text.strip()]
    
    This segment tries each seprator one at a time. 
    parts = [text]
    ...
    # It reassmbles the chunks with overlap. 
    chunks = []
    current_chunk = ""
    ...
    This is where it checks if the chunks have any empty strings in them. 
    if current_chunk:
        chunks.append(current_chunk.strip())
